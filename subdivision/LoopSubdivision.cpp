 ...
 */
void applyVertexRule(const WingedEdge* sourceMesh, WingedEdge* targetMesh,
    int vertexIndex) {
    //Apply Vertex rule
 ...
 */
void applyEdgeRule(const WingedEdge* targetMesh, int addedVertexCount, Edge* const* edgesCW,
 ...
 */
void updateVertices(const WingedEdge* sourceMesh, bool useVertexRule,
    WingedEdge* targetMesh) {// Apply vertex rule
 ...
 */
void subdivideEdges(const WingedEdge* sourceMesh, bool useEdgeRule,
    WingedEdge* targetMesh, ExtendedEdge* extendedEdges,
            ...
            if (!(extendedEdges[edgesCW[e] - sourceMesh->getEdges()].added)) {
                ...
                if (useEdgeRule) {
                    ...
                    targetMesh->getVertices()[addedVertexCount].setPosition(
                        edgesCW[e]->mVertOrigin->getPosition() * 0.5f + edgesCW[e]->mVertDest->getPosition() * 0.5f);
                }
 ...
 */
void subdivideFaces(const WingedEdge* sourceMesh, WingedEdge* targetMesh,
 ...
 */
std::vector<glm::ivec3> computeFaceIndices(const WingedEdge* originalMesh, const WingedEdge* subdividedMesh,
    ...
    for (int f = 0; f < originalMesh->getFaceCount(); f++) {
        ...
        for (int k = 0; k < 3; k++) {
            ...
            faceIndices[addedFaceCount++] = glm::vec3( c3 - subdividedMesh->getVertices(), c2 - subdividedMesh->getVertices(),
                c1 - subdividedMesh->getVertices());
        }
        ...
        faceIndices[addedFaceCount++] = glm::vec3( d3 - subdividedMesh->getVertices(), d2 - subdividedMesh->getVertices(),
            d1 - subdividedMesh->getVertices());
    }
    return faceIndices;
}
 ...
 * @return
 */
std::vector<glm::vec3> computeVertexIndices(const WingedEdge* subdividedMesh) {
 ...
 */
void Subdivision::loopSubdivision(WingedEdge* tMesh, const WingedEdge* sourceMesh, bool useVertexRule, bool useEdgeRule) {
    WingedEdge _targetMesh;
    WingedEdge* targetMesh = &_targetMesh;
